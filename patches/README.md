...
- `003-terminal-main-js` — (1) hotkey passthrough: whitelist tab-switch / new-terminal / settings / zen-mode command ids so they reach Obsidian while a terminal is focused; (2) focus-on-activate: switching to a terminal pane moves keyboard focus into the xterm input, guarded by the Electron window's real `isFocused()` (falling back to `document.hasFocus()`, which stays `true` in an unfocused Wayland window and would otherwise let an `xterm.focus()` raise Obsidian over the operator's current app). (The Alt+Ctrl+T binding itself lives in the tracked `hotkeys.json` — tracked config needs no log entry.)
...
- `006-terminal-start-offscreen` — the emulator start awaits `Ne(el)`, a CSS-readiness check implemented with an `IntersectionObserver`; that observer never fires for a hidden, occluded, or other-workspace window, so a terminal opened while Obsidian is off-screen never spawned its shell (Orbh launches from Strike hit this constantly, and the only workaround was raising the window). The edit keeps the intent — prove the plugin stylesheet applied — but reads it with `getComputedStyle`, falling back to the observer only when the class rule has not applied yet.
